...
//const landlordAttrs = ["email", "userName", "id"]
...
export function signUpUser({email, username, password}){
	...
	//	const attributeUserName = new CognitoUserAttribute(dataUserName)
		...
		//attributeList.push(attributeEmail, attributeUserName)
		...
		cognitoUser.authenticateUser(authenticationDetails, {
	        ...
	        onSuccess: function (result) {
	            ...
	            AWS.config.credentials.refresh(function(){
								console.log('Credential Mine : ');
	            	console.log(AWS.config.credentials);
	            })
							// Get API key before moving on
							const ApiUrl = 'http://34.237.44.133:10001/api/v1';
						  axios.get( ApiUrl + '/web/slipstream/private/jwt_test')
							 .then((response) => {
								 console.log(response.data.auth);
								 localStorage.setItem('KEY', response.data.auth.username+':'+response.data.auth.password+':'+response.data.auth.namespace);
								 const KEY = localStorage.getItem('KEY');
								 console.log("KEY_COG", KEY);
